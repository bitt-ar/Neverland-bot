...
import { createRedirectUrl, getDiscordOAuthUrl, isAuthEnabled, OAUTH_STATE_COOKIE, sanitizeRedirectPath } from "@/lib/auth";
...
export async function GET(request: NextRequest) {
  ...
  if (!isAuthEnabled()) {
    return NextResponse.redirect(createRedirectUrl("/overview", request));
  }
  ...
  const authUrl = getDiscordOAuthUrl(statePayload, request);
